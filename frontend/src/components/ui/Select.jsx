...
// Promoted from Cashbook.jsx — uses fixed dropdown to escape stacking contexts
export default function Select({ value, onChange, options, minWidth = 120, placeholder, style = {} }) {
  ...
  return (
    <div style={{ position: 'relative', minWidth, ...style }}>
      <button
        ref={btnRef}
        type="button"
        onClick={() => setOpen(v => !v)}
        style={{
          ...
          background: open ? 'var(--primary-subtle)' : 'var(--surface-solid)',
            ...
            {options.map(opt => {
              ...
              return (
                <div
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = active ? 'var(--primary-subtle)' : 'transparent' }}
                >
                  <span>{opt.label}</span>
                  {active && (
